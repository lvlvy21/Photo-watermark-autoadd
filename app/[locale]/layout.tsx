...
import { jetbrainsMono, playfairDisplay, roboto } from "@/lib/fonts.server";
...
export async function generateMetadata({ params }: Omit<LocaleLayoutProps, "children">): Promise<Metadata> {
  const locale = locales.includes(params.locale as (typeof locales)[number]) ? params.locale : defaultLocale;
...
export default async function LocaleLayout({ children, params }: LocaleLayoutProps) {
  ...
  return (
    <NextIntlClientProvider locale={locale} messages={messages}>
          ...
          />
          <div dir={dir} className={`${roboto.variable} ${playfairDisplay.variable} ${jetbrainsMono.variable}`}>{children}</div>
